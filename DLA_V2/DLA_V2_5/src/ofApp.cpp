...
//    topCam.setFarClip(10000);
    ...
    enterThread = sqrt(2)*maxCorePos + 2.2 * maxfltRadius;
...
void ofApp::update(){
    ...
    if (autoZoom) {
        camDist = ofMap(aggPtlPos.size(), coreNum, 1000+1.5*aggPtlPos.size(), 500, 2000);
...
//    ofCircle(corePos[0],25);
    ...
    shader1.setUniform1f("alpha", 0.5 * sin(0.02 + PI + ofGetElapsedTimef()) + 0.5);
