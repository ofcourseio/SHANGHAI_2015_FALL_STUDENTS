...
void dlaSystem::fltPtlresetbyIndex(int index){
    ...
    ofVec3f randomTarget;
    randomTarget = randomPtInCircle(winCen, enterThread);
    ofVec3f tempSpeed;
    tempSpeed = randomTarget - tempPos;
    tempSpeed.normalize();
    tempSpeed = tempSpeed * ofRandom(minfltSpeed, maxfltSpeed);
...
ofVec3f dlaSystem::randomPtonCircle(ofVec3f cen, float maxRadius){
    ...
    ptOnCircle.z = 5*sin(10*ofGetElapsedTimef());
    return ptOnCircle;
    
}

//------------------------------------------------------------
ofVec3f dlaSystem::randomPtInCircle(ofVec3f cen, float maxRadius){
    degreeSeed = ofRandom(0, 2 * PI);
    
    ofVec3f ptOnCircle;
    float radiusRange = ofRandom(maxGenRadius);
    ptOnCircle.x = radiusRange * sin(degreeSeed) + winCen.x;
    ptOnCircle.y = radiusRange * sqrt(1 - u*u) * cos(degreeSeed) + winCen.y;
//    ptOnCircle.z = 0;
    ptOnCircle.z = 5*ofGetElapsedTimef()* sin(10*ofGetElapsedTimef());
    return ptOnCircle;
...
//        pts[i].z = 0.5 * R * sin(0.005*R) * theta2;
        pts[i].z = 0.7 * R * theta2;
    }
