...
//but framerate gets slow :not solved  5th Dec
//fix random point on a sphere;
//animating light;
...
//        cout << "seed:" << degreeSeed <<endl;
        ...
        u = ofRandom(-1,1);
        pX = maxGenRadius * sqrt(1 - u * u) * cos(theta);
        pY = maxGenRadius * sqrt(1 - u * u) * sin(theta);
        pZ = maxGenRadius * u;
        ofVec3f tempPos;
...
//                    fltPtlRadiusES.push_back(tempRaduis);
                    ...
                    if (fltPtlPosES[j].distance(winCen) > 1.5*maxGenRadius) {
                        ...
                        tempSpeed = winCen - tempPos;
                        tempSpeed.normalize();
                        tempSpeed = ofRandom(7.0f,13.0f) * tempSpeed;
                        //                    fltPtlSpeedS.push_back(tempSpeed);
                        fltPtlSpeedS[j].set(tempSpeed);
                        float tempRaduis;
                        tempRaduis = ofRandom(12, 17);
...
//    topCam.setDistance(600+20*ofGetElapsedTimef());
    if (aggPtlNum < aggPtlPos.size()) {
        aggPtlNumFlag = true;
        aggPtlNum = aggPtlPos.size();
    }
    
    ofVec3f lightPos;
    lightPos.set(winCen.x + 500*sin(ofGetElapsedTimef()), winCen.y + 500 * cos(ofGetElapsedTimef()),500 * sin(ofMap(ofGetElapsedTimef(), 0, ofGetElapsedTimef(), -1 * PI , PI)));
    lighttest1.setPosition(lightPos);
...
//                aggPtlSphere.draw();
//                totalMesh.addVertices(aggPtlSphere.getMesh().getVertices());
//                totalMesh = aggPtlSphere.getMesh();
